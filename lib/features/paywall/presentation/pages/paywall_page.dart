...
/// The app's one paid-upgrade surface, always presented as a
/// *dismissible popup* over whatever the reader was doing — see
/// [showPaywallPopup], which is the only way this should be pushed.
///
/// There is no onboarding funnel around it any more: the reader is
/// already in the app, already has a shelf, and can close this at any
/// point without losing anything. Every exit — close, a completed
/// purchase, a restore — therefore lands back exactly where they were.
///
/// "join now" and "start free trial" both purchase a real RevenueCat
/// [Package] — see [PurchasesService] — and only report success once
...
///   [SoftPillButton], the same pill treatment the bottom tab bar
///   wears — the CTA doesn't need a shape of its own to stand out,
///   just to be the only pill on screen;
...
class _PaywallPageState extends State<PaywallPage> {
  ...
  /// True once a purchase has actually activated the entitlement — the
  /// brief celebration that replaces the pitch on the way out.
  bool _purchased = false;

  /// Holds the popup open on that celebration. Cancelled on dispose so
  /// a reader who backs out first isn't chased by a pop into whatever
  /// route replaced this one.
  Timer? _celebration;
  ...
  /// Same exit as [_dismiss], but held open for a beat on the "welcome
  /// to PRO" state first — only a genuine new purchase earns the
  /// celebration; restoring a past purchase or closing the popup goes
  /// straight out.
  void _dismissAfterPurchase() {
    ...
    _celebration = Timer(const Duration(milliseconds: 1600), _dismiss);
  }
                ...
                child: SafeArea(
                  ...
                  child: LayoutBuilder(
                    builder: (context, constraints) {
                      ...
                      // A purchase that actually activated the
                      // entitlement takes the whole card over for a
                      // beat — there is nothing left to pitch, and
                      // popping instantly would leave the reader
                      // unsure anything happened.
                      if (_purchased) return const _PurchasedState();

                      return Column(
                        children: [
                          _Header(onClose: _dismiss, metrics: m),
...
/// What the card becomes for the moment between a confirmed purchase
/// and the popup closing itself. Deliberately not a screen of its own:
/// it is a state of this card, so nothing new is pushed onto a stack
/// the reader is about to pop off anyway.
class _PurchasedState extends StatelessWidget {
  const _PurchasedState();
...
const _chapters = <_Chapter>[
  (
    ...
    body:
        ...
        'where it belongs. Deeper stats, a card of your reading year to '
...
/// contrast the rest of the app uses), with wide padding on every
...
/// mix the rest of the app wears. Content starts right under the top
