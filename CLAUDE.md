...
- **settings** — everything that isn't reading, behind the one gear in `TopBar`: subscription state (RevenueCat Customer Center + restore), light/dark/system, the account section, licenses, and a `kDebugMode`-only plan override. No screen in the app uses a Material `AppBar` — settings included; it wears the same `SafeArea` + `AppSpacing.xl` gutter + lowercase `jetBrainsMono` heading as every tab. The account section is the only place an email is ever typed — signing up is an optional bonus offered here rather than a toll gate at the front door — and what it does is narrow: `SessionService.linkEmail` attaches an identity to the *existing* anonymous user via `updateUser(email:)` + `verifyOTP(OtpType.emailChange)`, so the uid — and therefore the shelf, the streaks, and the RevenueCat entitlement — survives. A passwordless sign-in would provision a second account and abandon the first; don't reach for one here. There is deliberately **no sign-out** anywhere: the uid *is* the shelf, so ending an anonymous session destroys a library rather than protecting one.
