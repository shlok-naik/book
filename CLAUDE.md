...
- **shell** — `RootShell` hosts the four top-level pages (memory, streaks, library, log) in an `IndexedStack` switched by a floating `BottomSwitcher`; log is the default tab. All four wear the same `TopBar` — page name flush left, settings gear top right — because the gear is the app's only route to settings and a reader who can't find it on the page they're on can't find it at all; the log page's own title is `'add'` (`TopBar(title: 'add', center: DatePill())`), independent of the `DatePill` still centred on the page. The tab bar is icon-only, so `BottomSwitcher`'s four `Semantics` labels (`Memory`/`Streak`/`Library`/`Today`) *are* the tab names — deliberately not the same word as the log page's own `'add'` title, since one names the tab and the other names what the page does; keep them one word and title case.
