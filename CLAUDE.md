...
- **onboarding** — a six-screen intro shown once per install: welcome → two tutorial pages → "pick a look" → a teaser → the founder's note → finish, then `pushAndRemoveUntil` into `RootShell`. It is a *tour, not a gate*, and asks for nothing — no name, no email, no password — because the reader's anonymous account already exists before the first frame. `test/onboarding/intro_flow_test.dart` asserts there isn't a single `TextField` in the whole flow; keep it that way, and put anything that needs typing in settings instead. "Seen" lives on the device in `OnboardingStore`, written by `FinishPage` on the way *out* so quitting halfway doesn't burn it — it cannot live on the session, since every reader is signed in from launch one. The "pick a look" question writes to `ThemeController`, the same notifier the appearance section of settings drives, so it is a shortcut to a setting rather than separate state. The tutorial pages run an endless marquee, so tests there use fixed pumps and `ensureVisible`, never `pumpAndSettle`.
- **paywall** — the app's one paid-upgrade surface, always a dismissible popup over whatever the reader was doing (`showPaywallPopup`, never `Navigator.push(PaywallPage(...))` directly). `RootShell` presents it exactly once per install, to readers who don't already have pro; the flag lives on the device in `IntroOfferStore` (`shared_preferences`). Every one of those checks fails *closed* — an unreadable flag, an unreadable entitlement, and an existing subscriber all mean "don't show it". Otherwise it is only ever reached by a reader who asked: the `get cactus pro` row in settings.
