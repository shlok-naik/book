...
- **memory** — the reader's saved notes on how a book made them feel, captured by `remember` and shown in `ProfilePage`'s own memory list. `MemoryController` (a `ChangeNotifier`, injected via `MemoryScope` the same way `LibraryController` is via `LibraryScope`) is the single mutation point; `remember`/`forget` are optimistic, same convention as every `LibraryController` command. Feeds `recommend`'s grounding context — see **logging** above — but is otherwise independent of the library feature.
